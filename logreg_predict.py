...
def main(args):
    ...
    table = table[table.get_numbers_features()].fillna(0).values
    ...
    with open(PREDICT_CSV, 'w') as file_descriptor:
        ...
        file_descriptor.write('\n'.join(f'{i},{pred}' for i, pred in enumerate(predict)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filename_data', '-f', dest='filename_data', action='store',
                        help='input data file', default=TEST_CSV_FILE)
    parser.add_argument('--index_col', dest='index_col', action='store', help='index_col')
